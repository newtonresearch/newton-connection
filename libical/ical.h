...
/*======================================================================
  ...
  $Id: icalderivedvalue.h.in,v 1.4 2001/03/31 17:41:57 ebusboom Exp $
  $Locker:  $
...
/*======================================================================
  ...
  $Id: icalderivedparameter.h.in,v 1.1 2001/03/17 16:47:02 ebusboom Exp $
  $Locker:  $
...
/* -*- Mode: C -*-
  ...
  $Id: icalderivedproperty.h.in,v 1.4 2001/03/31 17:41:57 ebusboom Exp $
...
icalcomponent* icalparser_parse(icalparser *parser,
				char* (*line_gen_func)(char *s, unsigned int size, void *d));
...
char* icalparser_get_line(icalparser* parser, char* (*line_gen_func)(char *s, unsigned int size, void *d));

char* string_line_generator(char *out, unsigned int buf_size, void *d);
...
void* icalmemory_new_buffer(unsigned int size);
void* icalmemory_resize_buffer(void* buf, unsigned int size);
...
void icalmemory_append_string(char** buf, char** pos, unsigned int* buf_size, 
			      const char* string);

/*  icalmemory_append_char is similar, but is appends a character instead of a string */
void icalmemory_append_char(char** buf, char** pos, unsigned int* buf_size, 
			      char ch);
...
int sspm_parse_mime(struct sspm_part *parts, 
		    ...
		    char* (*get_string)(char *s, unsigned int size, void* data),
		    void *get_string_data,
		    struct sspm_header *first_header
    );

void sspm_free_parts(struct sspm_part *parts, unsigned int max_parts);
...
int sspm_write_mime(struct sspm_part *parts,unsigned int num_parts,
...
icalcomponent* icalmime_parse(	char* (*line_gen_func)(char *s, unsigned int size, 
						       void *d),
				void *data);
