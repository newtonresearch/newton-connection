...
#include <string.h> /* For strncpy & unsigned int */
...
{
    char *buf;
    unsigned int size = (unsigned int)end - (unsigned int)start;
...
char* icalparser_get_next_paramvalue(char* line, char **end)
{
    ...
    if (next == 0){
	next = line+(unsigned int)strlen(line);
    }
                          ...
                          char* (*line_gen_func)(char *s, unsigned int size, void *d))
{
    ...
    char *line_p;
    struct icalparser_impl* impl = (struct icalparser_impl*)parser;
    unsigned int buf_size = impl->tmp_buf_size;
...
icalcomponent* icalparser_parse(icalparser *parser,
				char* (*line_gen_func)(char *s, unsigned int size, 
						       void* d))
{
