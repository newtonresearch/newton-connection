...
/* Create a new temporary buffer on the ring. Libical owns these and
   wil deallocate them. */
void*
icalmemory_tmp_buffer (unsigned int size)
{
    char *buf;
...
void 
icalmemory_append_string(char** buf, char** pos, unsigned int* buf_size, 
			      const char* string)
{
    char *new_buf;
    char *new_pos;
    ...
    data_length = (unsigned int)*pos - (unsigned int)*buf;    
    final_length = data_length + string_length; 

    if ( final_length >= (unsigned int) *buf_size) {
	...
	*buf_size  = (*buf_size) * 2  + final_length;
	...
	new_pos = (void*)(new_buf + data_length);
...
void 
icalmemory_append_char(char** buf, char** pos, unsigned int* buf_size, 
			      char ch)
{
    char *new_buf;
    char *new_pos;
